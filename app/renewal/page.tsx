...
export default function RenewalPage(){return <div className={s.page} id="top"><MotionExperience/>
  ...
  <main id="main">
    ...
    <section data-bridge className={s.bridge} aria-label="一粒から、暮らしへ"><div className={s.bridgeSticky}><div className={s.bridgePhoto}><Image src="/images/rice-serving-morning.png" alt="湯気の立つごはんをよそう手元のイメージ" fill sizes="100vw"/></div><div className={s.bridgeWords}><p>FROM A GRAIN TO A CONNECTION</p><h2><span>一粒から、暮らしへ。</span><span>その食卓から、つながりへ。</span></h2></div><svg className={s.connectingLines} viewBox="0 0 1000 180" fill="none" aria-hidden="true">{['#c9b66a','#c79286','#95ac91','#8aa5b4'].map((color,i)=><path key={color} pathLength="1" stroke={color} strokeWidth="1.5" d={`M -30 ${30+i*9} C 250 ${30+i*9}, 260 ${150-i*9}, 510 ${150-i*9} S 800 ${25+i*8}, 1030 ${75+i*10}`}/>)}</svg></div></section>
