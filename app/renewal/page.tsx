...
function Photo({src,alt,className="",priority=false}:{src:string;alt:string;className?:string;priority?:boolean}) {return <div className={`${s.photo} ${className}`}><Image src={`/images/${src}`} alt={alt} fill sizes={priority ? "100vw" : "(max-width: 700px) 100vw, 55vw"} priority={priority}/></div>;}
...
export default function RenewalPage(){return <div className={s.page} id="top">
  ...
  <main id="main">
    ...
    <section id="about" className={s.intro}>
      <Label>Our thoughts</Label><h2>健やかさを、<br/>いつもの暮らしの中に。</h2><p>食べること。動くこと。<br/>自分を整えること。心を休めること。</p><p>そして、誰かと話し、学び、つながること。<br/>その小さな積み重ねが、<br/>自分らしく生きる力になっていく。</p><p>一般社団法人 結は、食・動・美・心を結び、<br/>人とのつながりの中で、健やかな毎日を育てます。</p>
    </section>
    <section className={s.about} aria-labelledby="thought-title"><Photo src="hero-table.png" alt="自然光に包まれた日々の食卓のイメージ" className={s.aboutRice}/><div className={s.aboutBody}><Label>Small things, every day</Label><h2 id="thought-title">健康は、食事だけで<br/>できているわけではありません。</h2><div className={s.offsetCopy}><p>栄養のある食事をとること。<br/>無理なく身体を動かすこと。<br/>自分をいたわり、休む時間を持つこと。</p><p>正しいことを知っていても、<br/>続けるのが難しい日もあります。</p><p>だから、一人で抱え込まなくていい。<br/>誰かとつながりながら、<br/>自分のペースで整えていく。</p><p>結は、その人らしい健やかさを<br/>支える場をつくります。</p></div></div></section>
    <section className={s.circleSection} aria-labelledby="circle-title"><div className={s.circleText}><Label>MUSUBU WELLNESS CIRCLE</Label><h2 id="circle-title">人と人を結ぶ、<br/>健康づくりの輪。</h2><p>学ぶ。体験する。話す。気づく。<br/>そして、また続けてみる。</p><p>目指すのは、暮らしに小さな変化が残る健康づくり。<br/>管理栄養士の専門性を軸に、地域、企業、店舗、<br className={s.desktopBreak}/>専門家、参加する人たちを結びます。</p><p>自分の健やかさが、家族へ、地域へ。<br/>その輪を、一緒に育てていきます。</p></div><div className={s.circle} aria-label="食・動・美・心を中心に、人、地域、企業、専門家がつながる輪"><Mark/>{["食","地域","動","企業","美","専門家","心","人"].map((label,i)=><span key={label} className={s[`orbit${i}`]}>{label}</span>)}</div></section>
    <section id="elements" className={s.section}><div className={s.sectionHeading}><div><Label>FOUR ELEMENTS</Label><h2>食べる。動く。<br/>整える。休む。</h2></div><p>「あわじ結び」に込めた、人と人のつながり。<br/>どれか一つだけを頑張るのではなく、<br/>その人の暮らしに合わせて、やさしく整えていく。</p></div><div className={s.elements}>{elements.map(e=><article key={e.name} className={s.element} style={{"--element-color":e.color} as React.CSSProperties}><Photo src={e.image} alt={e.alt}/><div className={s.elementName}><span>{e.name}</span><small>{e.en}</small></div><h3>{e.title}</h3><p>{e.body}</p><p className={s.tags}>{e.tags}</p></article>)}</div></section>
